...
md_expenses <- read_csv('data/maryland_expenses.csv') |>
  clean_names() #cleans out weird column names that r doesnt like
...
# media_expenses_clean |> group_by(payee_name) |> summarize(sum(amount))
...
**A2. The spenders who were spending more money were running for governor or the House/Senate, but all spenders at the top were political. There were also spenders paying for ads to support a cause, but these spenders bought less ads.**
...
**A3. Big politicians like Wes Moore and Tom Perez are not listed, which is interesting considering they had lots of facebook ads. In reality, Moore was one of the biggest payers in general for expenses.**

```{r}
media_expenses_clean |>
  group_by(amount, committee_name) |>
  summarize(total_amount =sum(amount)) |>
  arrange(desc(total_amount))
...
**A4. The list is different, because it includes more than just facebook funding and changes the order. The top spenders on FB advertising are not the top overall spenders on online advertising. However, some of the top spenders of FB spending seem to have only spent their money on FB ads.**
...
**A5 I have noticed that the candidates running for larger seats spend more money on ads which does make sense, but that the amount of money spent does not necessarily dictate who wins or loses. For example . I think that there could be a good story out of why some of the campaigns only use facebook, or a breakdown of the top spenders and their advertising, particularly online, and compare it to age demographics of each site. Then, we could rope in the winners and losers as well and compare that to the data. We know a lot about the funding, but not about what every person ran for because of the names. We would have to look that up, but it would be interesting to take into account.**
