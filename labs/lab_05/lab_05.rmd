...
baltco_911_overdose_calls <- read_csv("tabula-Baltimore_County_Carey_Samantha_log_OD.csv", col_names = FALSE) |>
clean_names() |>
...
A1. There are 327 rows, and 7/14 and 10/04 had the highest number of overdose calls. I do not think there are any days with no overdose calls because there are over 365 calls, but there very well could be many on a single day
...
A2. It is less in the middle of the week, but peaks on the weekend and on Monday

```{r}
baltco_911_overdose_calls_dayofweek <- baltco_911_overdose_calls |>
  mutate(day_of_week = wday(date))  
  
baltco_911_overdose_calls_dayofweek |>
  group_by(day_of_week) |>
  summarize(total_calls = n()) |>
  mutate(pct_total = total_calls/4112*100) |>
...
A3. Silver Spring has the most calls, at 36. I think that the structure of the data being exact addresses worries me in terms of how accurate the location is, because if we had a bit less specific location it could be grouped differently.
...
A4. I think that it would be interesting to look into the locations more in depth, and figure out the causes of why there are so many incidents in 450 silver spring road/ what's going on there.
