...
md_statewide_county <- read.csv ('data/md_statewide_county.csv') |> 
  clean_names() |>
  mutate(geoidid = as.character(geoid))|>
  mutate(total_governor = moore + cox + lashar + wallace + harding + sefcik + gov_writein,
    ...
    percentage_brown = (brown/total_ag)*100, 
    percentage_difference = (percentage_moore - percentage_brown))
...
#Create the map
ggplot() +
  geom_sf(data=md_statewide_county, aes(fill=percentage_difference)) +
  theme_minimal()
```
