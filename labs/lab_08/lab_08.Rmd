...
```{r}
pg_call <- read.csv("data/prince_georges_2022_overdoses.csv")
...
pg_call_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,percentage_per_month), weight=percentage_per_month)) +
  labs(
    title="Percentage of Calls Per Month",
    x = "Month",
...
md_median_income_zip <- get_acs(geography = "zcta", variable="B19013_001", year=2019, state="MD")|>
  rename(zipcode = GEOID, median_income = estimate)
  
joined_data <- pg_call_zip|>
  left_join(md_median_income_zip, by == 'zipcode')|>
  select(zipcode,total_calls, median_income, moe)
  
joined_data|>
  ggplot(aes(x=median_income, y=total_calls)) + geom_point()+
    labs(
    title="Calls Per Week",
    x = "Week",
    y = "Number of Calls",
