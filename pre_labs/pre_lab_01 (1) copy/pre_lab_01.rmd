...
**Answer 60712 observations/records/rows and 8 variables/columns**
...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer: chr for id, dbl for seats**
...
In the space below, write the title of the classes from the first row of the "name" column. **Answer: directed research**
...
**Answer: first line is selecting the umd courses data set, the second is taking all the terms and grouping the results based on the term. The third and fourth lines add up all of the records in each term in the data set.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: the first rows do the same thing as the previous code, but I think the last line is arranging the results of the count_classes in descending order from greatest to least.**
...
**Answer: The last line organizes the classes in alphabetical order by department name**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: The last line of the code organizes the results by number of seats, in descending order from greatest number of seats in the classes to least number of seats. I think the 4th line of code adds up the total number of seats, and the 5th line arranges them in descending order based on the numbers. This changed the organization to the number of seats.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: This code is organized by total number of seats in descending order, which is what the last line of code deals with. The other lines of code that are not the same add in the other factors to the chart (class count, mean seats, and median seats). Mean seats and median seats calculates the mean and median seats using total seats. The top terms are the same just with more information given.**
...
**Answer: I changed it so that the minimum and maximum seats are shown, which adds different rows. It calculates it using the data. The rest of the set is the same, and is organized in descending order by number of seats.**
...
1.  Do number of seats in each class correspond with the number of sections offered?

2.  Is there a difference when you sort between median number of seats versus when you sort by mean number of seats per department?

3.  Do departments with more classes offer more seats?
