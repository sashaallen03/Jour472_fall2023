...
**Answer: It selected only the id and the title of the class, filtering out all other information. It simplifies it, and could be used in future code more easily.**
...
**Answer: It gives us every single class that is in the theater department, or every single class that has more than 15 seats. This leads to a much bigger number as the result**
...
**Answer: It is formatted in year month day instead of month day year, and uses dashes instead of slashes.**
...
**Answer: It is adding up the total for each month, which is why the day is always 01. It is organized in descending order starting in the highest total amount.**
...
**Answer: It is adding a column in for the total votes, and a column for the percentage of votes that Moore got compared to total number of votes**
...
**Answer: It changes the order (puts it in descending order) of highest percentage to lowest percentage.**
...
**Answer: I think it is just as useful because you are able to see where the least amount of voters voted for Moore versus the most. They are both important to understand.**
...
**Answer: we could use str_to_title which capitalizes the first letter of an entire column, or str_to_lower, which converts the entire column to lowercase.**
...
**Answer: This mutate statement is abbreviating the states by detecting the address, so that written out certain states will be converted to 2 letter abbreviations. All other results are shown as N/A.**
...
**Answer: This answer does make sense, because it was an MD expenditure data and more \$ would be spent from MD sources and donors. Because we did not include all states, there is a high number of N/As. Typos could also be included in this.**
...
1.  Could you abbreviate class names?

2.  Could you find percentage of classes with more than 15 seats per class?

3.  Could you add more abbreviations of states, or a way to stop typos, into the MD expenditures sheet?
