...
**A1: It pulls from a website rather than a dataset that we already have downloaded.**
...
**A2: There are 9,776 records in the dataset, the data is organized by date and time, and tracks different aspects of the earthquakes, from magnitude to location.**
...
**A3: It is the one with a 4.2 magnitude from 2023-09-18.**
...
**A4: There are 12 earthquakes greater than 6**
...
**A5: There are only 5 with a depth greater than 6 and a depth less than 20**
...
**A6: We used or to find ones that fit both**
...
**A7: 3423 happened there, we filtered by Alaska**
...
**A8: I grouped by type, and found out how many of each there were with summarise**
...
**A9: I was surprised that there are some in the negatives, I don't know what that means!**
...
**A10: You can use mutate to extract the hour**
...
**A11: You use mutate to create a new column that is added to the data based on the magnitude**
...
```{r}
earthquakes |> filter(serious == 'Yes' & 
type =='earthquake') |> group_by(hour) |>
  summarize(count= n()) |> arrange(desc(count))
```

**A12**: We can do a filter by seriousness, and also type (which is not necessary but helpful). We then group by number and then summarize, which counts up how many serious earthquakes there were for each hour.
