...
```{r echo=FALSE}
census_api_key("c4b6d5f557fdd710f546c349eadd47d75c37b8aa", install=TRUE)
```
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer: Howard County has the highest median income**
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer Something like "Median Household Income by County in Maryland"**
...
Is the difference between Calvert and Montgomery significant? **Answer: The difference is not significant, but the margin of error is definitely different.**

Is the difference between Howard and everyone else significant? **Answer: Yes it is**
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer: This shows that the median household income for students who immediately enroll in high school (over 60%) is way higher than the median household income for students who do not (less than 40%), and there are more zipcodes of kids going to college in higher income areas.**
...
**Task** What is another question you could ask of this data? **Answer: We could ask a question breaking down average income by zip code within a county, and pinpoint graduation rates and college rates based on more specific geographic location.**
