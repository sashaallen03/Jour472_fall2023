...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer Yes, there is the delimiter error, we are having trouble retrieving payments**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: The vendor name is the names, amount is money, date is time**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think is happening with this data? **Answer There are no zip codes with less than 5 characers. This gave us a list of 549 zips that are less than 5 digits, and gave us the count for how many times those digits were used. This shows us how many errors there are**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer There are categories with NA listed**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer: We can see how much was donated by zip code, organization, and who the orgs donated to.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer: The function cleans up the data, makes sure that the headers are all distinct and the same even if there were errors in data.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer: 2 are possible, important to understanding the data w the records.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer: shows specifically distinct records and removes the duplicates**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: You could transform the zip into something that only has 5 digits, ex. the dash and numbers after the dash are not included. Just includes digits 1-5 in column zip 5.**
