...
\^ We needed to make sure the columns had the same name in order to combine them
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer The registration for Democrats and other increases, which could signify a population increase.**

```{r}
Fredrick_voters <- county_voters_combined |> filter(COUNTY == 'Frederick')
```
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer. There are 24 which are the number of counties plus Baltimore**
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer It shows the racial demogaphics, and the number of people over 18 in the state**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer. Prince George's County had the highest percentage of Black residents over 18, as well as the highest percentaage of democrats. Overall, there is a strong correlation between race an thepercentage of democrats in eac county, with Somerset being sort of an outlier. However, the top three counties with the highest Black population also have the highest percentage of democrats in the state.**
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer: They do not correlate as much to the percentage of democrats**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer The datatype is zipcode presented in the form of a string.**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer: The datatype of the zipcode was numbers, I am a little confused about what changed.**
